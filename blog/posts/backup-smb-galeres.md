...
`.git/**` correspond aux objets git binaires, régénérables via `git clone`. Inutiles en backup.

`**/node_modules/**` correspond aux dépendances JS, qu'un `npm install` recrée à l'identique. Des Go de fichiers pour rien.

`**/.env/**` correspond aux virtualenvs Python, même histoire, régénérables.

`**/ModuleOutput/**` correspond, chez moi, à des données extraites par Autopsy (forensics). Volumineuses et reproductibles.

`--local-encoding None` est lié directement à la galère n°2, ça évite que rclone ré-encode les noms de fichiers et bute sur les caractères exotiques.
