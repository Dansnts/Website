...
Quand on gère un cluster Kubernetes à la main, on finit toujours par se poser la question : « est-ce que ce qui tourne dans le cluster correspond vraiment à ce que j'ai dans mon repo ? ». Un `kubectl apply` oublié, un patch fait en urgence un soir de panne, et le cluster diverge silencieusement de Git. Personne ne sait plus quel est l'état réel.
