---
layout: post.njk
title: "Monitoring maison avec Prometheus, Grafana et node_exporter"
...
- `node_exporter_collectors` liste les collecteurs activés, injectée dans la commande via une boucle Jinja. J'active `hwmon` (capteurs de température), `nvme` (santé des SSD), `diskstats` et `filesystem`, exactement ce qui m'intéresse sur un hyperviseur.
- `Restart=always` fait que si le process meurt, systemd le relance. Un exporter doit être « toujours là ».
...
- `hostNetwork` / `hostPID` font que le pod partage la stack réseau et l'arbre des processus de l'hôte, sinon il ne mesurerait que lui-même.
...
- `scrape_interval: 15s` fait que Prometheus interroge chaque cible toutes les 15 secondes.
