...
Tu vois déjà le problème se dessiner ? Le serveur qui autorise les ports dépend d'un port… qui a besoin de ce serveur pour s'ouvrir.
...
La seule issue à ce stade : un accès physique/console au switch pour désactiver 802.1X à la main. En pleine nuit, ce n'est pas idéal.
...
Ce filtre laisse passer uniquement la MAC du node K8s, et il vit **entièrement dans le switch**. Si FreeRADIUS est mort, ça change rien : le port du node reste ouvert. Le cycle est brisé.

> Les autres ports (`ether3-5`), eux, restent en vrai 802.1X. Seul le port qui héberge l'infra RADIUS est exempté. On ne sacrifie la sécurité que là où c'est strictement nécessaire.
...
`auth-timeout-action=allow` inverse le comportement par défaut. Normalement, pas de réponse RADIUS = port fermé (fail-closed). Là, on passe en **fail-open** sur ce port précis : le timeout ouvre le port. RADIUS peut tomber, le node garde le réseau, et peut redémarrer FreeRADIUS.

Le compromis est explicite : **fail-open = moins sûr** (si RADIUS tombe, ce port n'est plus protégé). On ne l'active que sur le port de l'infra critique, jamais sur les ports clients.
...
La sécurité par défaut (fail-closed) est la bonne — **sauf** pour le maillon dont dépend la sécurité elle-même. Là, il faut accepter un fail-open, sinon on se verrouille dehors.
...
- **Le pire moment est le seul moment.** Ces bugs ne se manifestent qu'en panne. Les tester volontairement (couper RADIUS et vérifier qu'on s'en sort) vaut mieux que les découvrir en vrai.
