...
La réponse tient dans le **CSI driver SMB**. Il permet à Kubernetes de monter des partages SMB comme des volumes, exactement comme n'importe quel PVC. Dans ce post.
...
Un secret par partage (`nas`, `media`, `logs`), chacun avec un utilisateur TrueNAS distinct. Cloisonnement voulu, un pod qui accède aux médias n'a pas les creds du partage de sauvegarde.
...
Avec `reclaimPolicy: Retain`, si le PVC est supprimé, les données **restent** sur le NAS. En homelab, on préfère toujours ça à `Delete`, pas de perte accidentelle.
...
Avec `accessModes: ReadOnlyMany`, le CronJob de backup lit ce partage sans jamais avoir besoin d'y écrire. Lecture seule, un pod compromis ne peut pas corrompre la source.

La chaîne vide dans `storageClassName: ""` dit à K8s « ne provisionne rien dynamiquement, utilise ce PV précis ». C'est le pattern du binding statique.
...
`canary` est la branche de développement du driver, instable, susceptible de casser d'un jour à l'autre sur un `apply`. Un matin, les montages SMB tombent sans que rien n'ait changé, juste parce que la `canary` a évolué.
