...
Brancher son homelab directement sur la box du FAI, c'est simple. Jusqu'au jour où une VM compromise se retrouve sur le même réseau que le téléphone, la TV et l'ordi du salon. La bonne pratique, c'est d'**isoler** le lab. Un réseau à part, séparé de la maison par un routeur qui filtre ce qui passe.

Chez moi, ce rôle est tenu par un **MikroTik RB750Gr3**, un petit routeur pas cher mais redoutablement complet (RouterOS). Dans ce post, on construit la segmentation complète.
...
Le MikroTik reçoit une IP fixe côté box, plutôt que du DHCP. Ça garantit que son adresse ne change jamais. C'est important, la box va lui faire des port forwards vers cette IP.
...
Les ports `ether2` à `ether5` sont regroupés dans un **bridge**. Ensemble, ils forment un seul réseau logique, comme un switch. Le bridge porte l'IP de la gateway du lab.
...
Le `dns-server=10.0.0.101` pointe vers Pi-hole. Toute machine du lab s'en sert comme résolveur, DNS et blocage de pub en un.
...
Un DNAT plus subtil laisse le **réseau maison** utiliser Pi-hole comme DNS.
...
Le `src-address=192.168.1.0/24` restreint l'accès. Seules les machines de la maison peuvent taper Pi-hole via le WAN, elles profitent du blocage de pub sans être *dans* le lab.
...
> `connection-nat-state=dstnat` est malin. Plutôt que de lister chaque service exposé une deuxième fois dans le forward, on autorise d'un coup tout ce qui a déjà été redirigé par un DNAT. Une seule règle couvre WireGuard, TeamSpeak et le DNS maison.
...
- **Sauvegarder la config.** `/export file=backup` génère un `.rsc` rejouable, à versionner dans Git comme le reste de l'infra. Accessoirement, `/system routerboard settings set silent-boot=yes` désactive les bips au démarrage.
